...
class PlayingCardView: UIView {
    ...
    private lazy var upperLefttCornerLabel: UILabel = createCornerLabel()
    ...
    private func centeredAttributedString(_ string: String, fontSize: CGFloat) -> NSAttributedString {
        // 1. fontSzie -> responsize
        let font = UIFont.preferredFont(forTextStyle: .body).withSize(fontSize)//not responsive yet
        UIFontMetrics(forTextStyle: .body).scaledFont(for: font) // responsive
    ...
    override func layoutSubviews() {
        ...
        configureCornerLabel(upperLefttCornerLabel)
        upperLefttCornerLabel.frame.origin = bounds.origin.offsetBy(dx: cornerOffset, dy: cornerOffset)
        
        configureCornerLabel(lowerRightCornerLabel)
        // translate (move to bottom right)
        // rotate  1pi = 180 degrees
        // scale
        ...
        lowerRightCornerLabel.frame.origin = CGPoint(x: bounds.size.width, y: bounds.size.height)
