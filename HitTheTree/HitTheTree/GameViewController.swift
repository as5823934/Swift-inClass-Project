...
//  HitTheTree
//
//  Created by ping tseng tsai on 2018-06-15.
...
class GameViewController: UIViewController {
    ...
    func setUpSound() {
        let sawSound = SCNAudioSource(named: "chainsaw.wav")
        ...
        let backgroundMusic = SCNAudioSource(named: "background.mp3")
...
extension GameViewController : SCNSceneRendererDelegate {
    func renderer(_ renderer: SCNSceneRenderer, updateAtTime time: TimeInterval) {
        ...
        var camaraPosition = selfieStickNode.position
        
        let camDamping: Float = 0.3
        
        let xComponent = camaraPosition.x * (1 - camDamping) + targetPosition.x * camDamping
        let yComponent = camaraPosition.y * (1 - camDamping) + targetPosition.y * camDamping
        let zComponent = camaraPosition.z * (1 - camDamping) + targetPosition.z * camDamping
        
        camaraPosition = SCNVector3(x: xComponent, y: yComponent, z: zComponent)
        selfieStickNode.position = camaraPosition
        
        motion.getAccelerometerData {
            (x, y, z) in self.motionForce = SCNVector3(x: x * 0.05, y: 0, z: (y + 0.8) * -0.05)
        }
...
extension GameViewController : SCNPhysicsContactDelegate {
    func physicsWorld(_ world: SCNPhysicsWorld, didEnd contact: SCNPhysicsContact) {
        ...
        if contactNode.physicsBody?.categoryBitMask == CategoryTree {
            ...
            let unHideAction = SCNAction.run{
                (node) in node.isHidden = false
            }
